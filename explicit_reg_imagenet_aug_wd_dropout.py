...
train_loader, val_loader = data_loader('/ssd_scratch/cvit/madhav/Imagenet-orig2/')
...
def fit(epochs, model, trainloader, testloader, optimizer,scheduler,st_epoch=0,step_count=0):
  ...
  for epoch in range(st_epoch,epochs):  
    ...
    scheduler.step()
    filename = '/ssd_scratch/cvit/madhav/augment_checkpoint/'+str(epoch+1)+'_checkpoint.pth.tar'
    torch.save({
            'epoch': epoch+1,
...
if not os.path.exists('/ssd_scratch/cvit/madhav/augment_checkpoint/'):
  os.makedirs('/ssd_scratch/cvit/madhav/augment_checkpoint/')
...
if resume_training == True:
	checkpoint =  torch.load('/ssd_scratch/cvit/madhav/augment_checkpoint/3_checkpoint.pth.tar')
